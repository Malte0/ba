...
def simple_strategy(player, middle_card: Card, card_deck: Card_Deck):
    ...
    if middle_card.value in [-2, -1, 0, 1, 2, 3]:
        if allCardsLowerThan(player.card_grid, 4):
            # replaces a random closed card
            closed_cards = find_closed_cards(player.card_grid)
            card_to_replace = random.choice(closed_cards)
            new_middle_card = player.card_grid[card_to_replace[0]][card_to_replace[1]]
            player.card_grid[card_to_replace[0]][card_to_replace[1]] = middle_card
        else:
            highest_card_cords = findHighestCardInGrid(player.card_grid)
            new_middle_card = player.card_grid[highest_card_cords[0]][highest_card_cords[1]]
            player.card_grid[highest_card_cords[0]][highest_card_cords[1]] = middle_card
    else: 
        ...
        if drawn_card.value < 4:
            if allCardsLowerThan(player.card_grid, 4):
                ...
                card_to_replace = random.choice(closed_cards)
                new_middle_card = player.card_grid[card_to_replace[0]][card_to_replace[1]]
                player.card_grid[card_to_replace[0]][card_to_replace[1]] = drawn_card
            else:
                highest_card_cords = findHighestCardInGrid(player.card_grid)
                new_middle_card = player.card_grid[highest_card_cords[0]][highest_card_cords[1]]
                player.card_grid[highest_card_cords[0]][highest_card_cords[1]] = drawn_card
        else:
            new_middle_card = drawn_card
            closed_cards = find_closed_cards(player.card_grid)
            choice = random.choice(closed_cards)
            random_closed_card = player.card_grid[choice[0]][choice[1]]
